...
\quesref{ques:main}. Suppose we have $w \in W$ and $a_1,\ldots,a_n \in T \cup
...
A counterexample to \quesref{ques:main} can only exist, if there
is an element $u \in w C_T$ and three distinct generators $s_1,s_2,s_3 \in
D_r(u)$ such that $u \ul {s_i} \notin w C_T$ for $i=1,2,3$.

\begin{proof}
According to \propref{prop:counterexample-simplification}.
\end{proof}
\end{lemm}

\begin{lemm}
\label{lemm:counterexample-simplification2}
A counterexample to \quesref{ques:main} can only exist, if there are three not
