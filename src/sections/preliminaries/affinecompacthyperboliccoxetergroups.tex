...
\begin{defi}
	An irreducible Coxeter systems $(W,S)$ is called \defword{affine} if it is one of those from Figure~\ref{fig:affine-coxeter-systems}. The affine Coxeter systems arise from the finite ones by appending another generator in a certain manner.
\end{defi}
...
\begin{defi}
	An irreducible Coxeter systems $(W,S)$ is called \defword{compact hyperbolic} if it is infinite, not affine and if for each $S' \subsetneq S$ the parabolic subgroup $W_{S'}$ is finite. In this paper the compact hyperbolic Coxeter systems are denoted with $X$, but note that this is not common in literature.
\end{defi}
