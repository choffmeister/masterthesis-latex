...
\begin{defi}
	The cardinality of $S$ is called the \defword{rank} of the building.
\end{defi}

\begin{defi}
	For each $s \in S$ we define $c,d \in C$ to be $s$-adjacent if and only if $\delta(c,d) \in \{e,s\}$. Then $(\mathcal{C}, (\sim_s, s \in S))$ is called the \defword{associated chamber system} to $(\mathcal{C}, \delta)$.
\end{defi}
...
So thin buildings are precisely those that contain exactly one apparment, i.e.\ are apartments themself.
