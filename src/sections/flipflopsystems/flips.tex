...
\begin{defi}
	...
	\begin{axioms}
		\axiomitem{Fl1} $\tilde \theta^2 = \id$,
		\axiomitem{Fl2} $\tilde \theta(\mathcal C_+) = \mathcal C_-$ and
		\axiomitem{Fl3} for $\varepsilon \in \{+,-\}, x,y \in \mathcal C_+$ and $z \in \mathcal C_-$ we have $x \sim y$ iff $\tilde \theta(x) \sim \tilde \theta(y)$ and $x \opp z$ iff $\tilde \theta(x) \opp \tilde \theta(z)$.
	\end{axioms}
	Then we call $\tilde \theta$ a \defword{building quasi-flip} of $\mathcal{C}$. If in addition
	\begin{axioms}
		\axiomitem{Fl3a} for $\varepsilon \in \{+,-\}, x,y \in \mathcal C_+$ and $z \in \mathcal C_-$ we have $\delta_\varepsilon(x,y) = \delta_{-\varepsilon}(\tilde \theta(x),\tilde \theta(y))$ and $\delta^*(x,z) = \delta^*(\tilde \theta(x),\tilde \theta(y))$, 
	\end{axioms}
...
\begin{defi}
	We call a building (quasi-)flip \defword{proper} if there is a chamber $c \in \mathcal{C}_+ \cup \mathcal{C}_-$ with $\delta^{\tilde \theta}(c) = e \iff l^{\tilde \theta}(c) = 0$.
\end{defi}
