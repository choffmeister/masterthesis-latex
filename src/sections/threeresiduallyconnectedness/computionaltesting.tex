...
\begin{theo}
	...
	\begin{proof}
		The cases from the table have been proved by a simple algorithm. In the calculated poset graph we iterate over all combinations of generator sets $K,S_1,S_2,S_3$ (and ignore some trivial combinations for that we already known that they cannot yield a counterexample to \axiomref{3RC}) and then walk the poset graph to extract the residues $w_K C_{S_{ij}}$ and $w_K C_T$. See the appendix for concrete details.
	\end{proof}
\end{theo}
