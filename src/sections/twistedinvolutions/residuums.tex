...
Note that this proposition could be strengthen by insisting on an exclusive or, since we cannot have both cases at the same time. By the prove of \ref{coro:no-triple-edges} we see, that we cannot have $w \ul s \ul t = w$, since double edges are always twisted conjugations. Hence having $w \ul s \ul t \succ w \ul s \prec w \succ w \ul t$ would contradict to the convexity from \ref{prop:rank-2-residuums-are-convex}.

\begin{coro}
	\typedlabel{coro:onesided-operations-only-at-top-or-bottom-end-of-twocycle}
	Let $w \in S$ and let $s,t \in S$ be two distinct generators. Suppose $w \ul s \ul t \prec w \ul s \prec w$. If $s$ acts by multiplication on $w$, then $wC_{\{s,t\}}$ is finite and $w$ is the unique maximal element.

	\begin{proof}
		We assumed, that $w \ul s \ul t \prec w \ul s$. Hence by \ref{prop:onesided-operations-only-at-top-or-bottom-end-of-twocycle} we must have $w \ul t \prec w$. Then \ref{prop:rank-2-residuums-are-convex} asserts that $wC_{\{s,t\}}$ is finite with $w$ is its unique maximal element.
	\end{proof}
\end{coro}
