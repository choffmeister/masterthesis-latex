...
\begin{prop}[Deletion property for twisted expressions]
	...
	\begin{proof}
		Choose $j$ minimal, so we have $s_1 \ldots \ul s_j$ is not reduced. By \ref{prop:exchanged-property-for-twisted-expressions} there is an index $i$ with $s_1 \ldots \ul s_j = s_1 \ldots \ul{\hat s}_i \ldots \ul s_{j-1}$.
	\end{proof}
\end{prop}