...
\begin{defi}
	Each $w \in W$ with $\theta(w) = w^{-1}$ is called a \defword{$\theta$-twisted involution} or just \defword{twisted involution}, if $\theta$ is clear from the context. The set of all twisted involutions in $W$ regarding $\theta$ is denoted with $\ti{\theta}(W)$. Often we just omit the Coxeter group and write $\ti{\theta}$, when it is clear from the context which $W$ is meant.
\end{defi}
...
\begin{exam}
	...
	Note that $\theta$ is no Coxeter system automorphism, but we can think of it as one if we identify $S \subset W$ with $S \times S \subset W \times W$. Then the set of twisted involutions is
...
\begin{defi}
	...
	Let $k \in \nn$ and $s_{i_j} \in S$ for all $1 \leq j \leq k$. Then an expression $w \ul s_{i_1} \ldots \ul s_{i_k}$ is called \defword{twisted $w$-expression}. In case $w = e$ we omit $w$, just write $\ul s_{i_1} \ldots \ul s_{i_k}$ and call it \defword{twisted expression}.
\end{defi}
...
\begin{lemm}
	...
	\begin{proof}
		Suppose $\ul s$ acts by multiplication on $w$. Then $\theta(s)ws = w$ and so $l(\theta(s)ws) = l(w)$. So let the other way around $l(\theta(s)ws) = l(w)$. \todo
	\end{proof}
\end{lemm}
...
\begin{lemm}
	...
	For all $w \in W$ and $s \in S$ it is $w \ul s \ul s = w$.
...
\begin{lemm}[Lifting property for $\ul S$]
	...
	\begin{proof}
		Whenever a relation comes from the ordinary \ref{theo:lifting-property}, we denote it with $<_{LP}$ in this proof.
		\begin{description}
