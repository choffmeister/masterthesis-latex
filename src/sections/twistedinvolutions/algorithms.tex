...
\begin{lemm}
	Let $k \in \nn$, $n = |\{ w \in \ti{\theta} : \rho(w) \leq k \}|$ and for $0 \leq i \leq k$ let $\rho_i = |\{ w \in \ti{\theta} : \rho(w) = i \}|$. Then $\ref{algo:twoa2} \in \mathcal{O}(n)$.
...
The following results can be used to further reduce the count of needed element comparisons, when calculating the poset graph of $Wk(\theta)$.
