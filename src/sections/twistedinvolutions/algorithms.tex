...
\begin{lemm}
	Let $w \in S$ and $s,t \in S$ be two distinct generators with $w \ul s \neq w \ul t$. Then the following table shows all possible $n \in \nn$ with $w(\ul s \ul t)^n = w$ regarding a certain $\ord(st)$.

	\begin{center}
		\begin{tabular}{c|ccccccc|c}
			$ord(st)$	& 2 & 3 & 4 & 5 & 6 & 7 & 8 & \todo \\
			\hline
			n 			& 2 & 3 & 2,4 & 5 & 2,3,4,6 & 7 & 2,4,6,8 & \textrm{First, Fifth, Sixst}	\\
			n 			& 2 & -- & 3 & -- & 2,4 & -- & 5 & \textrm{Second} \\
			n			& -- & 2 & -- & 3 & -- & 2,4 & -- & \textrm{Third, Fourth} \\
		\end{tabular}		
	\end{center}

	\begin{proof}
		In each case we get a $m$ with $w = (\ul s \ul t)^m$ from the proof of \ref{lemm:max-twisted-circle-height}. By \ref{coro:rank-2-residuum-gcd-n-m} any $n$ with this property has a non trivial divisor in common with $m$.
	\end{proof}
\end{lemm}
