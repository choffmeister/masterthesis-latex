...
\begin{lemm}
	...
	\begin{proof}
		In each case we get a $m$ with $w = (\ul s \ul t)^m$ from the proof of \ref{lemm:max-twisted-circle-height}. By \ref{coro:rank-2-residuum-gcd-n-m} any $n$ with this property has a non trivial divisor in common with $m$. The situation $w \ul s \ul t = w$ for $s \neq t$ can only occur, if $\ul s$ and $\ul t$ act by twisted conjugation on $w$ due to the proof of \ref{prop:no-triple-edges}, and if $\ord(st) = 2$ due to \ref{coro:double-edges-only-for-ord-2}.
	\end{proof}
\end{lemm}
