...
\begin{coro}
	...
	\begin{proof}
		...
		is a bijection. It remains to show, that the relation $\preceq_W$ satisfies \ref{defi:direct-product-of-posets}. Let $v_1,w_1 \in \ti{\id}(W_1)$, $v_2,w_2 \in \ti{\id}(W_2)$ and $v = v_1v_2 = \varphi(v_1,v_2), w = w_1w_2 = \varphi(w_1,w_2) \in \ti{\id}(W)$. Suppose $v_i \preceq_{W_i} w_i$ for $i=1,2$. Then
		\begin{align*}
		...
		\begin{align*}
			v	& = \ul s_1 \ldots \ul s_m \ul t_1 \ldots \ul t_n \\
				& \leq \ul s_1 \ldots \ul s_m \ul t_1 \ldots \ul t_n \ul s_{m+1} \ldots \ul s_{m'} \ul t_{n+1} \ldots \ul t_{n'} \\
				& = \ul s_1 \ldots \ul s_m \ul s_{m+1} \ldots \ul s_{m'} \ul t_1 \ldots \ul t_n  \ul t_{n+1} \ldots \ul t_{n'} \\
				& = w.
		\end{align*}
