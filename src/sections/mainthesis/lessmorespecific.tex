...
\begin{lemm}
	...
	\begin{proof}
		...
		\begin{enumerate}
			...
			\end{align*}
			Note that the last equivalence is not true in general.
			...
			\end{align*}
			Note that the two last equalities are not obvious. Suppose $v = \ul v_1 \ldots \ul v_r \in \ti{\theta}$ to be an arbitrary twisted expression. In general we cannot deduce the action of $\ul s$ on a subexpression $\ul v_1 \ldots \ul v_k$ from the action of $\ul s$ on $v$ itself. In our special case it is possible due to the first two parts of this lemma. \qedhere
		\end{enumerate}
	\end{proof}
\end{lemm}